...
- Se l’IOPort è libera, il sistema è in servizio e almeno uno tra slot1--slot4 è libero, cargoservice entra nello stato engaged, riserva uno slot libero senza criteri di ottimizzazione e restituisce al customer il nome dello slot riservato. 
...
#domanda[ Posizione dell'IOPort nella mappa. I requisiti indicano che il cargorobot sposta il container dall'IOPort a slot5, lasciando intendere che l'IOPort sia una cella praticabile. Come si colloca nella griglia? ] 

#domanda[ Richieste concorrenti. Il sistema deve bufferizzare più richieste di carico contemporanee, oppure una nuova richiesta ricevuta mentre il sistema è engaged viene semplicemente rifiutata / respinta con retrylater senza accodamento? Dai requisiti si interpreta che le richieste non siano bufferizzate: se il sistema è occupato, la risposta è immediata (retrylater o refused) senza code di attesa. ] 

#domanda[ Liberazione degli slot e aggiornamento della disponibilità. I requisiti descrivono il processo di carico dei container negli slot1-4, ma non specificano come venga gestita la liberazione di uno slot già occupato. Possiamo assumere che lo svuotamento degli slot sia effettuato da sistemi o operatori esterni al nostro sistema? In tal caso, con quale meccanismo viene notificato a cargoservice che uno specifico slot è stato liberato e può tornare disponibile per future prenotazioni? ] 

#domanda[ Ripristino dello stato Service working. I requisiti specificano che il sistema entra nello stato Out of service quando il sonar rileva una distanza D > DFREE per almeno 3 secondi. Non è invece specificata la condizione per il ritorno allo stato Service working. Il sistema deve tornare operativo non appena il sonar rileva D ≤ DFREE oppure è richiesto un ulteriore intervallo di stabilità prima del ripristino del servizio?] 
...
+ Se soddisfatte: stato engaged, prenotazione slot, notifica al customer. 
...
+ cargoservice comanda al cargorobot di spostare il container da slot5 allo slot riservato; il sistema torna disengaged. 

== Macro-componenti e natura software 

#iss-table( columns: (auto, 1fr, auto), [Componente], [Ruolo], [Stato], [cargoservice], [Orchestratore principale. Natura reattiva (richieste in arrivo) e proattiva (comandi al cargorobot, aggiornamenti display). Non può restare privo di controllo: da requisito deve rispondere e agire autonomamente.], [Da sviluppare], [cargorobot], [Entità che governa il DDR per la movimentazione fisica richiesta dal cargoservice. La scelta POJO vs microservizio è rimandata all'analisi: se fosse un POJO, il cargoservice gli cederebbe il controllo (trasferimento di controllo sincrono) e non potrebbe reagire ad altri eventi durante la movimentazione. Se invece fosse un microservizio, la comunicazione sarebbe asincrona e message-driven, con un disaccoppiamento molto maggiore. Forti motivazioni spingono verso la seconda opzione, ma la decisione è oggetto degli sprint successivi.], [Da chiarire / da sviluppare], [IOPort], [Interfaccia con il customer (pushbutton + display). Se realizzata come microservizio separato (rappresentata per esempio tramite una Single Page Application (SPA), il pushbutton come un button HTML e il display come textarea HTML.), le responsabilità sono completamente disaccoppiate da cargoservice e i due possono essere sviluppati in parallelo; l'unica interfaccia condivisa è il messaggio load_request definito in questo sprint.], [Dispositivo fisico fornito \ sw da sviluppare], [sonar], [Sensore di distanza.], [Dispositivo fisico fornito \ driver sw da sviluppare], [marker device], [Etichettatura in slot5. ], [Dispositivo fisico simulato fornito \ sw da sviluppare], [LED], [Indicatore stato engaged. Può essere fisico o virtuale (rappresentato ad esempio come un indicatore visivo lampeggiante, eventualmente nella stessa SPA dell’IOPort  ). La scelta dipende dall'infrastruttura disponibile.], [Fisico o virtuale \ sw da sviluppare], [hold], [Struttura dati per lo stato della stiva (occupazione slot). Passivo.], [Da sviluppare], ) 
...
#iss-table( columns: (auto, 1fr), [Contesto], [Componenti e responsabilità], [ctxCargoService], [Contiene l'attore cargoservice. Nucleo del comportamento richiesto e punto di orchestrazione del ciclo di carico.], [ctxIO], [Raggruppa le entità legate all'IOPort e ai dispositivi citati dai requisiti: ioport, sonar, led, markerdevice.], [ctxRobot], [Raggruppa le entità legate al cargorobot e alla movimentazione richiesta.], ) 
...
#iss-table( columns: (auto, 1fr, 1fr), [Scenario], [Precondizioni], [Risultato atteso], 
...
[Guasto sonar], [Sistema operativo; sonar misura D > D#sub[FREE] per >= 3 s], [Sistema Out of service; display "Out of service"], 

[Rilevazione container], [Sistema engaged; sensor area vuota; sonar misura D < D#sub[FREE]/2 per >= 3 s], [cargoservice avvia la movimentazione verso slot5], ) 
...
#iss-table( columns: (1fr, auto), [Nome e Cognome], [Matricola], [Davide Chirichella], [0001222371], [Gabriele Doti], [0001245897], [Daniele Maccagnan], [0001247340], ) 