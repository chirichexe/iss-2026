...
L'obiettivo dello Sprint 0 è formalizzare in modo preciso e non ambiguo i requisiti forniti dalla committente: 
#figure(
  image("../../shared/requisiti_committente.png", width: 100%),
  caption: [Requisiti del committente],
)

costruire un primo modello logico dei macro-componenti del sistema, evidenziare il _core business_, motivare la scelta del linguaggio di modellazione e definire un primo insieme di piani di test funzionali. 
...
QAK viene generato automaticamente codice Kotlin eseguibile, il che consente di disporre di un *primo prototipo osservabile già nello Sprint 0*, prima ancora di scrivere una riga di logica applicativa. 
E' possibile trovare maggiori informazioni sul linguaggio QAK al seguente #link("https://anatali.github.io/issLab2026/_static/docs/Protobook.pdf")[link]
...
Il cargorobot è l'entità software che dovrà governare un Differential Drive Robot (DDR): con questo termine si intende un tipo di robot 
mobile che per spostarsi usa due ruote indipendenti tra loro nei lati del suo "corpo". Il cargorobot si occuperà quindi di amministrare
gli spostamenti del DDR in questione. 
Il suo compito principale è quello di fare in modo che il DDR trasporti i container in specifici slot. Per container si intende semplicemente
un contenitore di forma, dimensione e contenuto non precisati.

Si ritiene opportuno rappresentare il cargorobot come un QAK Actor. Dovendo governare i movimenti del DDR all'interno della hold, necessita di un controllo autonomo per poter ricevere ed eseguire in modo asincrono le richieste di movimento.

== hold

La hold rappresenta lo spazio virtuale all'interno del quale opera il cargorobot. Ha una forma piatta e rettangolare e al suo interno ospita:

- Dispositivo di Input/Output chiamato IOPort, questa è dotata di:
...
- 5 slots rappresentanti specifiche aree della hold nelle quali si conservano i container, di questi 4 sono normali (slot1—slot4) e uno è "speciale" (slot5) ed è 
riservato per determinate funzionalità. 
...
Il Sensore è un dispositivo di rilevamento (un sonar) associato all'IOPort. Il suo compito è quello di rilevare la presenza di container da caricare e di comunicare
costantemente le sue rilevazioni. La distanza sarà calcolata basandosi su una precisa area della hold chiamata sensor_area.
...
Il marker è un dispositivo che si occupa di assegnare a un container, presente in quel preciso momento nello slot5, un determinato codice a barre. Dopodichè dovrà comunicare
la fine dell'operazione al sistema.

Si ritiene opportuno modellare il marker come un QAK Actor.

== cargoservice

- Il servizio da realizzare è *cargoservice*: il customer gli invia una richiesta di carico tramite il pushbutton dell'IOPort e cargoservice si occupa del funzionamento
generale dell'applicazione, in particolare dovrà soddisfare le seguenti indicazioni: 

- Ricevere e interpretare i dati del sonar. In particolare, definendo una distanza D e un'altra D#sub[free]:
  - Se il sensore misura D > D#sub[FREE] per >= 3 s dovrà mettere il sistema nello stato **Out of service**.
  - Se il sensore misura D < D#sub[FREE]/2 per >= 3 s significa che il sensore ha rilevato un container e il normale funzionamento può iniziare, quindi cargoservice 
  comanda al cargorobot di spostare il container *dall'IOPort* allo *slot5* (adibito al deposito temporaneo di un container prima della collocazione definitiva). 


- Gestire il funzionamento del pushbutton, se viene premuto: 
...
- Gestire il display dell’IOPort, facendo si che in ogni momento mostri: 
...
- Inoltre deve controllare che:
  - Fintanto che il sistema è _engaged_, un *LED lampeggia*. 
  - Dopo l'accettazione, il customer ha un tempo prefissato a 30 s per depositare il container nell'area del sonar. Se il customer non deposita entro il timeout, il sistema torna _disengaged_. 
