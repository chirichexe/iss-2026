...
L'uso del linguaggio qak ci permetterà di modellare il *cargoservice* come un *attore autonomo*. Il sistema si avvarrà dei seguenti collaboratori simulati:
  ...
  - *sonarmock*: simulerà il rilevamento fisico del container emettendo l'evento Event sonardata : distance(D) che, per notificare che l'area dell'IOPort è occupata, verrà ascoltato dal sistema. Simulerà inoltre eventuali eventi di guasto per forzare il sistema nello stato di Out of service.
