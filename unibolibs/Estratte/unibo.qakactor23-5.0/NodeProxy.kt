...
class NodeProxy( name: String, val ctx: QakContext, val protocol: Protocol,
    ...
    fun configure() {
        while (conn == null) {
            ...
            if (conn == null) {
                ...
                CommUtils.outgray("               %%% NodeProxy $name | in ${ctx.name} PROXY ACTIVATED TO $hostName:$portNum ")
    ...
    override suspend fun actorBody(msg: IApplMessage) {
        //CommUtils.outred("       NodeProxy $name receives $msg while conn=$conn ") // conn=$conn"
        try {
            conn?.forward("$msg")
        } catch (e: Exception) {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                while (true) {
                    ...
                    //CommUtils.outred("               %%% NodeProxy $name  | receives: $msg  ")
                    if( msg == null ){
                        break
                    }
                    sysUtil.traceprintln("               %%% NodeProxy $name  | receives: $msg")
                    ...
                    if( inputmsg.isEvent() ){  //Oct2019
                        ...
                        } else println("    %%% NodeProxy $name  | WARNING!! no  ${dest} in ${ctx.name}")
                    }
                }
            }catch (e: Exception) {
