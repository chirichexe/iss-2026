...
suspend fun sendMsg( sender : String, msgId: String, msg: String, destActor: ActorBasic) {
    ...
    @JvmStatic
    suspend fun sendRequest(sender : String, msgId: String, msg: String, destActor: ActorBasic) {
        val requestMsg = buildRequest(sender, msgId, msg, destActor.name)
        destActor.actor.send(requestMsg)
...
suspend fun emitEvent(msg: IApplMessage) {
    ...
    if( msg.isEvent()){
        ...
        if( emitter != null ) {
            CommUtils.outred("${emitter.name} emitEvent  $msg")
...
suspend fun sendMsg(  sender: String, msgId : String, payload: String, destName : String, mqtt: MqttUtils ){
    ...
    fun getConnection(protocol: Protocol, hostName: String, portNum: Int, clientName:String) : Interaction? {
        when( protocol ){
            Protocol.TCP -> {
                CommUtils.outcyan("               %%% MsgUtil | getConnection $hostName:$portNum")
                try {
                    val socket = Socket(hostName, portNum)
                    CommUtils.outcyan("               %%% MsgUtil | getConnection $socket")
